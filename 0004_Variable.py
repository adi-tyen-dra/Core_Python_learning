# variable is basically just a way to give a meaningful name to an area of memory, into wich we can place certain values.
# when a var is created python allocates a certain area to it in memory and knows where to call it when the var is called.
# it also allocates the type of var based on the kind of val provided and allows only the relevent action wrt the var type.
...
# in python values bound to the var have a data type instead of var itself hence type of var may change when val with a different data type is rebounded to the var.
...
# A strongly-typed programming language is one in which each type of data (such as integer, character, hexadecimal, packed decimal, and so forth) 
