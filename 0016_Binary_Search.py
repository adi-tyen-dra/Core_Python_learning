...
while True:
    ...
    if hi_lo == "h":
        ...
        pass #in a working environment when code is to be added laterr but not adding it gives error, pass can be used in such situations
    elif hi_lo == "l":
        #guess lower, higher limit = current guess -1
        pass
    elif hi_lo == "c":
        print("your guess is correct, it took us {} attempts to figure out".format(guesses))
    else:
...
low = 1
high = 1000
print("please think of a no. between {} and {}".format(high,low))
input("please hit enter")
guesses = 1
while True:
    print("\tguessing in the range of {1} and {0}".format(high,low)) #prints out the ranges that narrows down using binary search
    guess = low + (high-low)//2
    hi_lo = input("my guess is {} should i guess high or low"
                  "enter either h, l or c respectively"
                  .format(guess)).casefold()
    if hi_lo == "h":
        ...
        print("thank you, it took us {} attempts to figure out".format(guesses))
        break
    else:
        print("please enter h, l or c")
    guesses += 1
